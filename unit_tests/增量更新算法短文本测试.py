...
class StreamWidthCalculatorTest(TestApp):
    def build(self):
        ...
        self.short_text_btn = Button(text="短文本测试",font_name="STSONG", on_release=self.start_short_text_test)
        self.control_panel.add_widget(self.start_btn)
        self.control_panel.add_widget(self.pause_btn)
        self.control_panel.add_widget(self.short_text_btn)
    ...
    def start_stream_test(self, *args):
        ...
        self.short_text_btn.disabled = True
    ...
    def start_short_text_test(self, *args):
        """开始短文本测试"""
        self.testing = True
        self.start_btn.disabled = True
        self.short_text_btn.disabled = True
        ...
        self.incremental_errors = 0
        self.total_chars = 0
        self.status_label.text = "短文本测试进行中..."
        
        # 用于收集结果
        results = []
        error_count = 0
        
        for i, text in enumerate(self.short_text_samples):
            # 重置标签状态
            self.test_label._last_text = ""
            self.test_label._last_max_width = 0
            self.test_label._last_line_width = 0
            ...
            for char in text:
                ...
                # 计算宽度
                incremental_width = self.test_label.incremental_calc_width(current_text)
                full_width = self.test_label.full_calc_width(current_text)
            ...
            # 最终比较
            final_inc = self.test_label.incremental_calc_width(text)
            final_full = self.test_label.full_calc_width(text)
            diff = abs(final_inc - final_full)
            ...
            results.append(
                f"文本 {i+1}: [{text}] -> "
                f"增量: {final_inc:.1f}, 全量: {final_full:.1f}, "
                f"差异: {diff:.2f} {match}"
            )
            
            # 更新UI
            self.results_label.text = (
                f"已完成 {i+1}/{len(self.short_text_samples)} 个短文本测试\n"
                f"当前错误: {self.incremental_errors} ({self.incremental_error_rate:.2f}%)"
            )
        
        # 计算错误率
        self.incremental_error_rate = (error_count / len(self.short_text_samples)) * 100 if self.short_text_samples else 0
        
        # 显示最终结果
        results_str = "\n".join(results)
        self.status_label.text = "短文本测试完成!"
        self.results_label.text = (
            f"测试结果 ({len(self.short_text_samples)} 个样本):\n"
            f"最终错误率: {self.incremental_error_rate:.2f}%\n"
            f"错误样本数: {error_count}\n\n"
            "详细结果:\n" + results_str
        )
        ...
        self.short_text_btn.disabled = False
        self.pause_btn.disabled = False
        
        # 控制台输出
        print("\n" + "="*50)
        print("短文本测试摘要:")
        print(f"测试样本数: {len(self.short_text_samples)}")
        print(f"错误样本数: {error_count}")
        print(f"错误率: {self.incremental_error_rate:.2f}%")
        print("详细结果:")
        for res in results:
            print(res)
        print("="*50)
