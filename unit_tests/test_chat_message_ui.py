...
class Test(TestApp):
    def build(self):
        ...
        self.com.add_widget(MDBoxLayout(
            ImeTextInput(callback=add_item,multiline=True),
            #md_bg_color=input_color,
            size_hint=(1,0.1),
            size_hint_x=1,
...
if __name__ == "__main__":
    ...
    Test().run()
    # c.add_message(role="user",context="你好")
    # c.add_stream_finish_callback(lambda u,x:print(x,len(x),end=''))
    # c.stream()
    # c.join()